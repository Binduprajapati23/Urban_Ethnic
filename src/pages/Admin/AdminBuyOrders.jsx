...
const BUY_ORDERS_MIGRATION_KEY = "buy_orders_db_migrated_v1";
...
const AdminBuyOrders = () => {
  const [orders, setOrders] = useState([]);
  const [rentalOrdersCount, setRentalOrdersCount] = useState(0);
  ...
  useEffect(() => {
    void requestJson(`${API_BASE}/api/admin/notifications/mark-read`, {
      method: "POST",
      body: JSON.stringify({ types: ["new_buy_order"] }),
    });
  }, []);

  const readLegacyBuyOrders = useCallback(() => {
    try {
      const raw = JSON.parse(localStorage.getItem(ADMIN_ORDERS_KEY) || "null");
      const allOrders = Array.isArray(raw) ? raw : [];
      return allOrders.filter((order) => {
        const orderType = String(order?.type || "buy").trim().toLowerCase();
        const items = Array.isArray(order?.items) ? order.items : [];
        const hasBuyItems = items.some((item) => String(item?.mode || "buy").trim().toLowerCase() === "buy");
        return orderType === "buy" || orderType === "mixed" || hasBuyItems;
      });
    } catch {
      return [];
    }
  ...
  const migrateLegacyOrdersToDatabase = useCallback(async () => {
    const isAlreadyMigrated = localStorage.getItem(BUY_ORDERS_MIGRATION_KEY) === "1";
    if (isAlreadyMigrated) return;

    const legacyBuyOrders = readLegacyBuyOrders();
    if (legacyBuyOrders.length === 0) {
      localStorage.setItem(BUY_ORDERS_MIGRATION_KEY, "1");
      return;
    }

    try {
      await requestJson("http://localhost:5000/api/admin/buy-orders/sync", {
        method: "POST",
        body: JSON.stringify({ orders: legacyBuyOrders }),
      });
      localStorage.setItem(BUY_ORDERS_MIGRATION_KEY, "1");
    } catch (err) {
      console.log("Buy orders migration failed:", err?.body || err.message);
    }
  }, [readLegacyBuyOrders]);
  ...
  const fetchBuyOrders = useCallback(async () => {
    ...
    try {
      ...
      console.log("Failed to fetch buy orders:", err?.body || err.message);
      setError("Unable to load buy orders from database.");
      setOrders([]);
    } finally {
      setIsLoading(false);
    }
  ...
  useEffect(() => {
    const bootstrapOrders = async () => {
      ...
      await Promise.all([fetchBuyOrders(), fetchRentalOrdersCount()]);
    };
    bootstrapOrders();
  }, [fetchBuyOrders, fetchRentalOrdersCount, migrateLegacyOrdersToDatabase]);

  const buyOrders = useMemo(() => orders, [orders]);
  ...
  const filteredOrders = useMemo(
    () =>
      buyOrders.filter((order) => {
        ...
        const matchesType = normalizedType === "all" || normalizedType === "buy";
        const matchesStatus = statusFilter === "all" || order.status === statusFilter;
        return matchesSearch && matchesType && matchesStatus;
      }),
    [buyOrders, searchQuery, statusFilter, typeFilter]
  );

  const stats = useMemo(() => {
    const totalOrders = buyOrders.length;

    return [
      { key: "total", label: "Total orders", value: totalOrders.toLocaleString("en-IN") },
      { key: "buy", label: "Buy orders", value: totalOrders.toLocaleString("en-IN") },
      { key: "rent", label: "Rental orders", value: rentalOrdersCount.toLocaleString("en-IN") },
    ];
  }, [buyOrders, rentalOrdersCount]);
  ...
  const exportCsv = useCallback(() => {
    const rows = filteredOrders.map((order) => {
      ...
      const type = String(order?.type || "Buy").trim() || "Buy";
                ...
                filteredOrders.map((order) => {
                  ...
                  return (
                    <tr key={orderId} className="bg-white hover:bg-[#E6E6E6] transition-colors">
                      ...
                      <td className="py-4 px-4">
                        <span className="px-2.5 py-1 text-[10px] font-bold uppercase rounded-full border bg-slate-100 text-slate-700 border-slate-200">
                          Buy
                        </span>
                      </td>
